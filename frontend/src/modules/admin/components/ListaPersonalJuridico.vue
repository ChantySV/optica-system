...
<template>
  <div class="container mx-auto mt-8 bg-white p-6 rounded-lg">
    <!-- Título -->
    <h1 class="text-3xl font-bold text-gray-700 mb-6 text-center"> Personas Jurídicas</h1>
...
const loadPersonas = async () => {
  ...
  try {
    ...
    const response = await findAllJuridicos(itemsPerPage, offset, sortField.value, sortOrder.value);
    if (response.ok && response.data) {
      personas.value = response.data.juridicos;
      totalItems.value = response.data.total;
    } else {
      toast.error(response.message || 'Error al cargar las personas jurídicas.');
    }
  } catch (err) {
    console.error(err);
