...
import { findProducto, findTramiento, findColor, findPersonal, updateTrabajo } from '../actions/trabajos.action';
...
const formData = reactive({
  fecha_salida: props.trabajo.fecha_salida,
  numero_trabajo: props.trabajo.numero_trabajo,
  id_personal: props.trabajo.id_personal,
  detalleTrabajo: {
...
async function onSubmit() {
  ...
  try {
    const response = await updateTrabajo(props.trabajo.id_trabajo , formData);
    if (response.ok) {
      toast.success('Trabajo actualizado con éxito');
      emit('updated'); // Emitir evento de actualización
      closeModal();
    } else {
      toast.error(response.message || 'Error al actualizar trabajo');
    }
  } catch (error) {
    console.error(error);
