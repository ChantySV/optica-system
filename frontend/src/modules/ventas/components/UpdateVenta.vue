    ...
    <div class="bg-white p-6 w-full max-w-4xl rounded-lg shadow-md">
      ...
      <div class="mt-6">
        ...
        <ul v-else class="space-y-2">
          ...
          >
            <span class="text-gray-700">
              <strong> Numero Trabajo: {{ trabajo.numero_trabajo }}</strong>
            </span>
            <button
...
import { updateVenta, findPendientes, findPrecioVenta, searchPersonaVenta } from '../actions/venta.action';
import { useToast } from 'vue-toastification';

const props = defineProps({
  isOpen: Boolean,
  // La venta seleccionada se pasa como propiedad para precargar los datos
  venta: {
    type: Object,
    default: null,
  }
...
const onSubmit = async () => {
  ...
  try {
    const response = await updateVenta(props.venta.id_venta, {
    ...
    if (response.ok) {
      ...
      emit('ventaActualizada'); // Notificar al componente padre que se actualizó la venta
      closeModal();
    } else {
      toast.error(response.message || 'Error al actualizar la venta.');
    }
  } catch (error) {
    console.error(error);
...
// Cargar datos de la venta seleccionada en el formulario
const cargarDatosVenta = async () => {
  if (props.venta) {
    formData.value.monto_total = Number(props.venta.monto_total);
    formData.value.id_persona = props.venta.id_persona || '';
    formData.value.nombre_persona = props.venta.personal || '';
    formData.value.trabajosSeleccionados = [];

    if (props.venta.detalleVentas && props.venta.detalleVentas.length > 0) {
      for (const detalle of props.venta.detalleVentas) {
        try {
          const precioVenta = await findPrecioVenta(detalle.id_trabajo);
          formData.value.trabajosSeleccionados.push({
            id_trabajo: detalle.id_trabajo,
            numero_trabajo: detalle.numero_trabajo,
            precio_sugerido: precioVenta.ok && precioVenta.data ? precioVenta.data : 0,
          });
        } catch (error) {
          console.error(error);
          toast.error('Error al cargar precio del trabajo.');
        }
      }
    }
  }
...
// Vigilar cambios en la propiedad "venta" para actualizar los datos del formulario
